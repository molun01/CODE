...
void restart(int bk[20][20], int look[4], int xy[2], int move[20][20])
{
	...
	int pp, qq, i, j; //用来记录果实的随机坐标，范围是1-15
	...
	for (i = 0; i <= 16; i++)
		...
		xy[1] = xy[0] = 8;
		bk[xy[0]][xy[1]] = 3;
		move[xy[0]][xy[1]] = 1; //则此时8,8,上的轨迹就应该记录为1
		move[0][0] = 1;			//用此来记录步数
		look[0] = 1;			//表示朝向,向上
		look[1] = 1;			//表示长度
		look[2] = 0;			//当为1是表示失败
		...
		for (i = 0; i <= 16; i++)
			...
			printf("现在得分是:%d,请再接再厉!^_^", look[1] - 1);
}
...
void map(int bk[20][20], int look[4], int xy[2], int move[20][20])
{
	...
	int b[10], qq = 0, pp = 0, i, j; //b用来吸收输入，qq和pp用来随机初始化果实坐标
	if (_kbhit())			   //记录按下的是哪个方向键，方向键的ASCLL值:上72，左75，右77，下80
	{
		b[0] = _getch(); //用b来记录
		if (b[0] == 224)
			b[0] = _getch();				//如果为224表示为方向键，但是要再一次获取才行
		if (b[0] == 72 && look[0] != 2) //如果输入的为上并且朝向不为下
			look[0] = 1;
		if (b[0] == 80 && look[0] != 1)
			look[0] = 2;
		if (b[0] == 75 && look[0] != 4)
...
int main()
{
	...
	handle = GetStdHandle(STD_OUTPUT_HANDLE); //获取控制台的句柄
