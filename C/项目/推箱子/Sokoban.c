	...
	while (1)
	{
		...
		if (t == '1')
		{
			...
			if (strcmp(passwords, Passwords) == 0)
				...
				MessageBox(NULL, TEXT("\t    密码错误！！！\t\n\t请重新输入密码。\t"), TEXT("输入提示"), MB_OK);
		}
		else
			Introduction(ch);
	}
...
void Introduction(char* ch)
{
	...
	{
		MessageBox(NULL, TEXT("\t          请输入密码。\t\n\t（少于十个数字或字母）\t"), TEXT("输入提示"), MB_OK);
		Password(ch);
		Autor(ch);
	}
...
void GameOperate(int Map[][20], int* n, int* m, int* js, int* sum, int* data, int* sl, int* guanshu, char* ch)//游戏函数调用
{
	...
	while (1)
	{
		...
		*sum -= *sl;
		...
		if (*ch == '*') 
		{
			int i = MessageBox(NULL, TEXT("\t\n确定要退出游戏吗？\n\n（提示：退出后重新进入从退出的关卡开始）。"), TEXT("退出提示"), MB_YESNO);
			if (i == IDYES) 
			{
				*bushu = 0;
				kongge = 0;
	...
	while (1) 
	{
		...
		*sum -= *sl;
		if (*sum == *data)
		{
			...
			printf("\t\t\t\t\t    恭 喜 通 关！\n");
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_RED | FOREGROUND_INTENSITY);
			t = MessageBox(NULL, TEXT("\t      你真棒！！！\n\n\t是否进入下一关？"), TEXT("提示"), MB_YESNO);
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_GREEN | FOREGROUND_INTENSITY);
			printf("\t\t 通关所用步数为%d步。\n", *bushu);
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_BLUE | FOREGROUND_INTENSITY);
			printf("\t       请稍等,正在创建下一关。");
